...
function NounsGridContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const selectionModeParam = searchParams.get("select") === "true";
  const { searchQuery, setSearchQuery, debouncedSearchQuery, urlLevel } = useDebouncedSearch("/nouns");
  ...
  useEffect(() => {
    if (displayedNouns.length > 0) {
      saveNavigationList("nouns", displayedNouns.map(n => n.id), "/nouns");
    }
  }, [displayedNouns]);

  // Toggle noun selection
  const toggleNoun = (id: string) => {
    setSelectedNouns((prev) => {
  ...
  // Create collection with selected nouns
  const createCollection = () => {
    const ids = Array.from(selectedNouns).join(",");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <div className="mb-6">
          ...
          <p className="text-muted mt-2">
            Essential nouns for everyday Japanese communication
          </p>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="mt-4 text-sm text-muted">
            {isLoading
              ? "Loading..."
              : searchQuery
                ? `${displayedNouns.length} nouns found`
                : totalCount
                  ? `${totalCount} nouns`
                  : ""}
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-bold text-foreground">No Nouns found</h3>
            <p className="text-muted mt-2">
            ...
            <button
              onClick={() => {
                setSearchQuery("");
                router.replace("/nouns", { scroll: false });
              }}
      ...
      {selectionMode && selectedNouns.size > 0 && (
        <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-40">
          ...
          >
            Create Collection ({selectedNouns.size})
          </button>
        </div>
      )}
    </div>
...
export default function NounsGridPage() {
  return (
    <Suspense
      fallback={
        <div
        ...
        >
          <div className="text-6xl text-white animate-pulse">名</div>
        </div>
      }
    >
      <NounsGridContent />

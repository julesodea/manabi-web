...
export default function NounDetailPage() {
  ...
  const { currentId, hasPrev, hasNext, goToPrev, goToNext } = useNavigationList("nouns", initialId);
  const { data: noun, isLoading, error } = useNoun(currentId);
          ...
          <div className="mb-4 lg:mb-8 flex flex-col justify-between gap-3">
            <div>
              <h1 className="text-2xl lg:text-4xl font-bold text-foreground mb-2 lg:mb-4">
                {(noun.meaning.charAt(0).toUpperCase() + noun.meaning.slice(1)).replace(/;\s*/g, ", ")}
              </h1>

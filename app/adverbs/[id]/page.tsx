...
import { useAdverb } from "@/lib/hooks/useAdverbs";
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <div className="text-6xl mb-4 animate-pulse text-[var(--accent)]">副</div>
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center bg-card-bg rounded-3xl p-8 shadow-xl max-w-md mx-4 border border-border">
          <p className="text-muted mb-4">Adverb not found</p>
          <button
            onClick={() => router.back()}
      ...
      <main className="flex items-center justify-center px-4 pt-20 pb-4 h-screen overflow-hidden">
        <div className="bg-card-bg rounded-3xl p-6 lg:p-8 shadow-xl border border-border max-w-4xl w-full max-h-full overflow-y-auto flex flex-col justify-evenly relative">
          ...
          <div className="mb-4 lg:mb-8 flex flex-col justify-between gap-3">
            <div>
              <h1 className="text-2xl lg:text-4xl font-bold text-foreground mb-2 lg:mb-4">
                {(adverb.meaning.charAt(0).toUpperCase() + adverb.meaning.slice(1)).replace(/;\s*/g, ", ")}
              </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-5">
              ...
              {adverb.romaji && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 lg:gap-3">
                    <span className="px-3 py-1.5 lg:px-4 lg:py-2 rounded-lg text-base font-medium bg-card-bg text-foreground border border-border">
                      {adverb.romaji.charAt(0).toUpperCase() + adverb.romaji.slice(1)}
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="space-y-2 lg:space-y-4">
            {adverb.example_sentences && adverb.example_sentences.length > 0 ? (
              adverb.example_sentences.map((sentence, i) => (

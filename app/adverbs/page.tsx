...
function AdverbsGridContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const selectionModeParam = searchParams.get("select") === "true";
  const { searchQuery, setSearchQuery, debouncedSearchQuery, urlLevel } = useDebouncedSearch("/adverbs");

  const [selectedAdverbs, setSelectedAdverbs] = useState<Set<string>>(new Set());
  const [selectionMode, setSelectionMode] = useState(selectionModeParam);
  const [menuOpen, setMenuOpen] = useState(false);

  // TanStack Query hooks
  const { data: totalCount } = useAdverbsCount(urlLevel);
  ...
  // Create collection with selected adverbs
  const createCollection = () => {
    const ids = Array.from(selectedAdverbs).join(",");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <div className="mb-6">
          ...
          <p className="text-muted mt-2">
            Essential adverbs for everyday Japanese communication
          </p>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="mt-4 text-sm text-muted">
            {isLoading
              ? "Loading..."
              : searchQuery
                ? `${displayedAdverbs.length} adverbs found`
                : totalCount
                  ? `${totalCount} adverbs`
                  : ""}
          </div>
        ...
        {isLoading ? (
        ...
        ) : displayedAdverbs.length === 0 ? (
          <div className="text-center py-20">
            <div className="text-6xl mb-4">🔍</div>
            <h3 className="text-xl font-bold text-foreground">No Adverbs found</h3>
            <p className="text-muted mt-2">
            ...
            <button
              onClick={() => {
                setSearchQuery("");
                router.replace("/adverbs", { scroll: false });
              }}
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {displayedAdverbs.map((n) => {
                const isSelected = selectedAdverbs.has(n.id);
                ...
                return selectionMode ? (
                  <button
                    key={n.id}
                    type="button"
                    onClick={() => toggleAdverb(n.id)}
                    className="text-left"
                  >
                    {cardContent}
                  </button>
                ) : (
                  <Link key={n.id} href={`/adverbs/${n.id}`}>
                    {cardContent}
                  </Link>
                );
              })}
      ...
      {selectionMode && selectedAdverbs.size > 0 && (
        <div className="fixed bottom-10 left-1/2 transform -translate-x-1/2 z-40">
          ...
          >
            Create Collection ({selectedAdverbs.size})
          </button>
        </div>
      )}
    </div>
...
export default function AdverbsGridPage() {
  return (
    <Suspense
      fallback={
        <div
        ...
        >
          <div className="text-6xl text-white animate-pulse">副</div>
        </div>
      }
    >
      <AdverbsGridContent />
